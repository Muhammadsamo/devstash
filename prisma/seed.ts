...
const systemItemTypes = [
  { name: 'Snippet', slug: 'snippet', category: ContentType.TEXT, icon: 'Code', color: '#3b82f6', isSystem: true },
  { name: 'Prompt', slug: 'prompt', category: ContentType.TEXT, icon: 'Sparkles', color: '#8b5cf6', isSystem: true },
  { name: 'Command', slug: 'command', category: ContentType.TEXT, icon: 'Terminal', color: '#f97316', isSystem: true },
  { name: 'Note', slug: 'note', category: ContentType.TEXT, icon: 'StickyNote', color: '#fde047', isSystem: true },
  { name: 'File', slug: 'file', category: ContentType.FILE, icon: 'File', color: '#6b7280', isSystem: true },
  { name: 'Image', slug: 'image', category: ContentType.FILE, icon: 'Image', color: '#ec4899', isSystem: true },
  { name: 'Link', slug: 'link', category: ContentType.URL, icon: 'Link', color: '#10b981', isSystem: true },
]
...
async function main() {
  ...
  for (const type of systemItemTypes) {
    ...
    if (!existing) {
      ...
      itemTypeMap[type.slug] = created.id
      console.log(`   ✓ Created: ${type.name}`)
    } else {
      itemTypeMap[type.slug] = existing.id
